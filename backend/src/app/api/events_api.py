...
#         if meal_two:
#             results_score_map[meal_two] = inf
#             specified_mealtimes = True
#         if meal_three:
#             results_score_map[meal_three] = inf
#             specified_mealtimes = True
...
#     zero_checker = [0 for _ in range(51)]
#     if user_preferences == zero_checker:
#         user_preferences = [(random.randint(-10, 10)/10) for _ in range(51)]

#     res = {}
#     events = [e for e in event_results if e.get("end_date") != '' and e.get("address") != '' and e.get("lat") and e.get("long")]
#     for event in events:
#         #calculate the event score based on the user's tags(in order)
#         # Score: dot product of tag_scores and user_preferences

#         tag_scores = event.get("categories")
#         score = sum(p * t for p, t in zip(user_preferences, tag_scores))
#         res[event["title"]] = (event, score)
    
#     sorted_event_scores = sorted(res.values(), key=lambda item: item[1], reverse=True)
...
#     events = [event[0] for event in results_score_map]
#     scores = [event[1] for event in results_score_map]
#     n = len(events)
...
#             start1, end1 = datetime.fromisoformat(e1.get("start_date")), datetime.fromisoformat(e1.get("end_date"))
#             start2, end2 = datetime.fromisoformat(e2.get("start_date")), datetime.fromisoformat(e2.get("end_date"))
            
#             # Calculate travel time between events
#             travel_time = timedelta(minutes=fetch_travel_time(e1.get("lat"), e1.get("long"), e2.get("lat"), e2.get("long")))
...
#                 start_time, end_time = datetime.fromisoformat(event.get("start_date")), datetime.fromisoformat(event.get("end_date"))
                
#                 # Create meal window times on the same date as the event
#                 meal_start = start_time.replace(hour=window_start_hour, minute=0, second=0)
#                 meal_end = start_time.replace(hour=window_end_hour, minute=0, second=0)
...
#                  for i in range(n)
#             ]) <= window_duration - 0.5, f"{meal_label}_constraint"  # Changed from -1 to -0.5
